        ...
        <form action="{{ route("admin.atendimentos.update", [$atendimento->id]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('procedimento') ? 'has-error' : '' }}">
                ...
                <select id="procedimento" name="procedimento" class="form-control" required>
                    ...
                    @foreach(App\Atendimento::PROCEDIMENTO_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('procedimento', $atendimento->procedimento) === (string)$key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group {{ $errors->has('duracao') ? 'has-error' : '' }}">
                ...
                <select id="duracao" name="duracao" class="form-control" required>
                    <option value="" disabled {{ old('duracao', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                    @foreach(App\Atendimento::DURACAO_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('duracao', $atendimento->duracao) === (string)$key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
