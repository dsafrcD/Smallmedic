        ...
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-Paciente">
                ...
                <tbody>
                    ...
                    @if($paciente->Medico == Auth::user()->name)
                        <tr data-entry-id="{{ $paciente->id }}">
                            <td> </td>
                            <td>{{ $paciente->id ?? '' }}</td>
                            <td>{{ $paciente->nome ?? '' }}</td>
                            <td>{{ $paciente->nascimento ?? '' }}</td>
                            <td>{{ App\Paciente::SEXO_RADIO[$paciente->sexo] ?? '' }}</td>
                            <td>{{ $paciente->email ?? '' }}</td>
                            <td>{{ $paciente->fone_pessoal ?? '' }}</td>
                            <td>{{ $paciente->fone_comercial ?? '' }}</td>
                            <td>{{ $paciente->endereco ?? '' }}</td>
                            <td>{{ $paciente->bairro ?? '' }}</td>
                            <td>{{ $paciente->cidade ?? '' }}</td>
                            <td>{{ $paciente->cep ?? '' }}</td>
                            <td>{{ $paciente->estado ?? '' }} - {{ $paciente->Medico ?? '' }}</td>
                            <td> @if($paciente->documento) @foreach($paciente->documento as $key => $media) <a href="{{ $media->getFullUrl() }}" target="_blank">{{ trans('global.view_file') }}</a><br> @endforeach @endif</td>
                            <td> @can('paciente_show')<a class="btn btn-xs btn-primary" href="{{ route('admin.pacientes.show', $paciente->id) }}">{{ trans('global.view') }}</a>@endcan
                                 @can('paciente_edit')<a class="btn btn-xs btn-info" href="{{ route('admin.pacientes.edit', $paciente->id) }}"> {{ trans('global.edit') }}</a>@endcan
                                 @can('paciente_delete')
